...
Future<Uint8List> generateSimplePdf({
  ...
  _pdfCallCount++;
  final callNo = _pdfCallCount;
  developer.log('generateSimplePdf çağrıldı #$callNo', name: 'pdf_service_simple');
  ...
  // Değişkenlerin scope'u: fonksiyonun geri kalanında kullanılabilir
    ...
    for (final b in kat.bolumler) {
      if (b.isOrtakAlan) {
        ...
        if (b.sahip == 'Muteahhit') {
          muteahhitVar = true;
          final trp = b.girilenToprakParasi;
          toplamToprakParasi += trp;
  ...
  developer.log('Toprak özeti:', name: 'pdf_service', error: {
    'toplam_toprak': toplamToprakParasi,
    'mal_sahibi_sayisi': malSahibiSayisi,
    'kisi_basi_iadesi': kisiBasiToprakIadesi,
    'toprak_sutunu_var': toprakSutunuVar,
  });
